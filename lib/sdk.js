    ...
    if (prefix) {
        if (stat.isFile()) {
            ...
            if (ext && ext === path.extname(prefix)) {
                ...
                objectName = path.sep + path.dirname(prefix) + path.sep + basename;
            }
            else {
                // edp bos lib/bos.js bs://adtest/hello/world
                // objectName = '/hello/world/bos.js'
                objectName = path.sep + prefix + path.sep + this._getBaseName(localFile);
            }
        }
    }
    else {
        objectName = path.sep + this._getBaseName(localFile);
    }

    return objectName.replace(/\/+/g, path.sep);
};
...
BaiduObjectStorage.prototype.upload = function (bucketName, localFile, prefix) {
    ...
    objectName = objectName.replace(/\\+/g, path.sep);
...
BaiduObjectStorage.prototype.realUpload = function (localFile, bucketName, objectName) {
    ...
    objectName = path.sep + objectName;
    objectName = objectName.replace(/\/+/g, path.sep);
    ...
    this.client.putObject(bucketName, objectName, data, {
        ...
        var ep = me.endpoint;
        var bosUrl = (endpointmap.hasOwnProperty(ep) ? endpointmap[ep] : ep) + path.sep + bucketName + objectName;
