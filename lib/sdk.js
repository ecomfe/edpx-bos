 ...
 */
BaiduObjectStorage.prototype._getCdnEndpoint = function (endpoint) {
    ...
    if (prefix) {
        if (stat.isFile()) {
            ...
            if (ext && ext === path.extname(prefix)) {
                ...
                objectName = '/' + prefix + '/' + this._getBaseName(localFile);
            }
        }
    }
    else {
...
BaiduObjectStorage.prototype.realUpload = function (localFile, bucketName, objectName) {
    ...
    this.client.putObject(bucketName, objectName, data, {
        ...
        var bosUrl = me._getCdnEndpoint(me.endpoint) + '/' + bucketName + objectName;
        edp.log.info(bosUrl);
        def.resolve(bosUrl);
    }).fail(function () {
